...
//    b.digitalWrite('P8_13', b.LOW);
 ...
 b.analogWrite(SERVO, 0.01);
 b.analogWrite(SERVO1, 0.01);
 b.analogWrite(SERVO2, 0.1);
 b.analogWrite(SERVO3, 0.1);
 b.analogWrite(SERVO4, 0.2);
 b.analogWrite(SERVO5, 0.2);
...
//			console.log("past:"+xt[current]+",now:"+sumtemp);
	...
	//	  }	 
		...
		//	else {
	         ...
	         b.analogWrite(SERVO2, 0);
	         b.analogWrite(SERVO3, 0);
		//	}
		  //   if (position > 0){
		  //      position1 =  position;
		//	    updateDim()
...
function scheduleNextUpdate() {
    ...
    if(position < 0) {
        ...
        increment = -increment;
	     ...
	     b.analogWrite(SERVO2, position);
		 b.analogWrite(SERVO3, position);
...
function updateDim() {
	...
	if (position<0.01){
		...
		b.analogWrite(SERVO2, 0.01);
		b.analogWrite(SERVO3, 0.01);
	  //  b.analogWrite(SERVO3, 0.01, null,scheduleNextDim);
		b.analogWrite(SERVO, 0);
         b.analogWrite(SERVO1, 0);
	}
	else {
	...
	b.analogWrite(SERVO1, position);
	b.analogWrite(SERVO2, position);
	b.analogWrite(SERVO3, position, null,scheduleNextDim);
    }
...
function updateDuty() {
    ...
    var duty_cycle = position;
	b.analogWrite(SERVO, duty_cycle);
	b.analogWrite(SERVO1, duty_cycle);
	if (position<0.05){
		b.analogWrite(SERVO2, 0.05);
	b.analogWrite(SERVO3, 0.05, null,scheduleNextUpdate);
	}
    else {	
   b.analogWrite(SERVO2, duty_cycle);
	b.analogWrite(SERVO3, duty_cycle, null,scheduleNextUpdate);
    }
